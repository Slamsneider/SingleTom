...
<details>
  ...
  **I repeat that this tutorial project is aimed at local use only and ONLINE deployment is not the scope of this tutorial project.**
  ...
  The important thing is to not expose your API key to the world. So instead you make an api call to a server that can do the OpenAI API CALLs for you while not exposing the API key to the user.
